...
if(!empty($_FILES['image']['name'])){
  ...
  $studentQuery = "UPDATE activity_tb SET title='$title', description='$description',date='$today',image='$filename' WHERE id='$id' ";
  $result = $classhelper->db_con->query($studentQuery);

}
else
{
  $studentQuery = "UPDATE  activity_tb SET title='$title', description='$description',date='$today' WHERE id='$id'";
  $result = $classhelper->db_con->query($studentQuery);
}     
if($result){
...
?>
  ...
  <?php
  header("Location:manageactivity.php?status=update"); 
  ?>
...
$stmt = $classhelper->db_con->prepare("SELECT * FROM `activity_tb` WHERE id=$sliderid");
$stmt->execute();
$users = $stmt->fetchAll();
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <textarea name="description" class="form-control ckeditor" placeholder="Enter Description"><?php echo $description; ?></textarea>
                  </div>
